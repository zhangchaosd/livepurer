...
**方式二(推荐, 供 IPTV/APTV 等播放器作为稳定源):**

传入 `plat` + `room`，由服务端在每次播放时动态获取最新流地址并转发，避免原始流地址签名过期导致的播放中断。
...
请求示例：`/api/v1/live/play?plat=douyu&room=6556593`

在局域网内的 IPTV / APTV 等播放器中配置该地址即可观看，例如：

```
http://192.168.1.100:8800/api/v1/live/play?plat=douyu&room=6556593
```
...
部分平台不支持某些指标时，接口仍返回其他可用数据；缺失指标不会填充为 0，并在 `data.unavailable` 中列出字段名。全部指标读取失败时返回错误。
...
`GET /api/v1/os/all` 额外返回 `data.lan_ip`：运行服务器的局域网 IPv4 地址，优先 `192.168.0.0/16`，其次其他私有 IPv4；无可用地址时为空字符串。仅检查已启用的非回环网卡，排除公网、链路本地及 IPv6 地址。


### 直播封面图片

`GET /api/v1/live/cover?plat=huya&room=226046`（也支持 HEAD）返回 `image/jpeg`，可直接用于电视播放器的图片加载，无需自定义请求头。

- 优先读取平台封面/预览图，失败或格式无法解码时使用主播头像，再失败则返回内置占位图。
- 代理获取图片时附带平台 Referer，支持 JPEG、PNG、GIF、WebP 输入并统一输出 JPEG；AVIF 暂不解码，会回退头像。
- 不打开直播流、不调用 FFmpeg，不是服务端实时截图。更新频率取决于平台及播放器缓存。
- 最大输出尺寸 640×360，服务端正常图片缓存 60 秒，占位图缓存 15 秒。支持 `ETag` / `If-None-Match`，`X-Cover-Source` 标识 `cover`、`avatar` 或 `placeholder`。
- 参数仅接受支持的平台和房间标识，不接受任意图片 URL。上游目标与重定向均进行公网地址校验，并限制下载大小、像素数、并发数和缓存容量。

M3U 的 `#EXTINF` 现在默认包含 `tvg-logo="http://<局域网IP>:<端口>/api/v1/live/cover?plat=...&room=..."`，普通频道与收藏夹订阅均生效。手工配置的 `logo` 仍优先原样输出；留空即可使用自动封面。已有电视订阅需重新拉取播放列表，必要时清除播放器图标缓存。

`/live/room_info` 与 `/live/room_infos` 的房间数据新增 `cover`、`avatar` 字段，分别提供平台原始封面和头像地址；电视端推荐使用上述本地 JPEG 接口。
